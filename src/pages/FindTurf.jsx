...
import { SPORTS, CITIES, TURFS } from '../data';
...
function FindTurf() {
  ...
  const filtered = TURFS.filter(t=>!sport||sport==="All"||t.s.some(s=>s.toLowerCase().includes(sport.toLowerCase())));
  return (
    <main className="pg" role="main" aria-label="Find a Turf">
      <div className="pghero">
        ...
        <div className="pghero-inner">
          ...
          <p className="a3" style={{fontSize:16,color:"var(--muted)",maxWidth:480,lineHeight:1.65}}>500+ verified turfs across 12 cities and 12 sports. Book in under 60 seconds.</p>
        </div>
      </div>
            ...
            {searched&&(
              <div className="slide">
                ...
                {filtered.length===0?(
                  ...
                  <div className="g3">{filtered.map((t,i)=><TurfCard key={i} t={t} onBook={()=>{}}/>)}</div>
                )}
              </div>
            )}
          </div>
